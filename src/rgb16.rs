...
use anyhow::Result;
use ndarray::{array, ArrayView1, IndexLonger};
...
fn normalize(x: u16) -> f32 {
    x as f32 / u16::MAX as f32
}

pub fn to_sdr(raw: Vec<u16>, hight: u32, width: u32) -> Result<Vec<u8>> {
    println!("{:?}", &raw[0..6]);
    // shape the raw buffer in to a 3D ndarray
    // The shape of the ndarray is (height, width, 3)
    let rgb = ndarray::Array3::<u16>::from_shape_vec((hight as usize, width as usize, 3), raw)?;
